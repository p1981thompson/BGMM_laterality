...
keywords          : "Bayesian, Gaussian Mixture Model, Language, Hemispheric lateralisation, Online testing"
wordcount         : "X"
...
The Gaussian mixture model with $m$ components for the response variable $Y_{i}$ given the covariates $\boldsymbol{x}_{i}, i=1,2,\ldots,n.$, has the conditional distribution as follows:
...
where $\phi_{k}$ is a vector of mixing proportions provided $\sum_{k=1}^{m}\phi_{k}=1$, $\phi_{k}>0$, $k=1,2,\ldots,m$, $\beta_{k}$ are $k$ vectors of model parameters, and $\sigma_{k}$  are the $k$ variance parameters. We also define $y$ as observations from our response variable and $x$ as a set of covariates. This model requires that the error terms are independent and identically distributed, and follow a normal distribution with mean zero and constant variance. 
...
then, the observations are drawn from their corresponding subpopulations, given the values of $z_{i}$,

$y_{i}|z\sim f(\cdot|\theta_{z_{k}})$, for $i=1,2,\ldots, n.$


 where $\theta = (\mu,\sigma,\phi)$ are the model parameters.

We fit the model within a Bayesian framework, so we specify the following priors:

$$
\mu_{k} \sim \mathcal{N}(\mu_{0},1/B0inv);
\sigma_{K} \sim Lognormal(\mu_{0},\tau_{0})
\phi_{k} \sim Uniform(0,1);
$$

### Label switching problem solution

When using a Bayesian framework for the analysis of mixture models, label switching is a familiar problem. The 
